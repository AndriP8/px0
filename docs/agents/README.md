...
| `web/src/pr.js`        | GitHub PR review, active only when `S.meta.pr` is set: the PR bar (Approve / Request Changes / Comment, draft count), the inline draft comment composer (registers into `selbar.js` via `setReviewHandler`, mirroring `agent.js`), gutter comment markers (registers into `diff.js` via `setPRSyncHandler`), and `launchPR()` for the Command Palette's "Open Pull Request…" (used by `palette.js`). |
