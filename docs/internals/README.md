...
- [GitHub PR Review](github-pr-review.md): Zero-dependency REST client and three-source auth resolution, temp-dir worktree checkout scoped to the process lifetime, merge-base diffing instead of `HEAD`, fail-closed push-access gating, and the in-memory (never persisted) draft comment and review-submission model.
