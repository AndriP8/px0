...
| **GitHub PR Review** | `px0 pr <n>`, `Alt+R` | Full-tree checkout of a pull request, merge-base diffing, and draft comments with Approve / Request Changes / Comment | [GitHub PR Review](github-pr-review.md) |
...
- **[GitHub Pull Request Review](github-pr-review.md)**: `px0 <pr-url>` checks out a PR's full source tree into its own process, diffs it against the PR's merge-base instead of `HEAD`, provides real-time progress spinning, detects already-merged PRs with confirmation prompts, and lets you draft inline review comments, batch-apply them with coding agents, or submit Approve / Request Changes / Comment back to GitHub.
